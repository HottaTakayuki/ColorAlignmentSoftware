...
| 呼出し先 | 役割 | 同期/非同期 |
|----------|------|--------------|
| `UpdateAccSettings` | カメラ接続前の設定ファイルを更新する | 同期 |
| `StartCameraController` | AlphaCameraController の起動を保証する | 同期 |
| `ChechCcProcess` | 制御プロセスの起動状態を確認する | 同期 |
| `CameraCorrectionData.LoadFromXmlFile` | カメラ補正データを読込む | 同期 |
| `LedCorrectionData.LoadFromXmlFile` | LED補正データを読込む | 同期 |
