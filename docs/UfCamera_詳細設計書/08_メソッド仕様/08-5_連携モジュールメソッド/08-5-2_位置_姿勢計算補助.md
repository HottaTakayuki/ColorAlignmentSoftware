...
| 呼出し先 | 役割 | 同期/非同期 |
|----------|------|--------------|
| `sendSdcpCommand` | 温度補正値・低輝度モード設定を読出す | 同期 |
...
| 概要 | カメラと選択Cabinet群の相対位置を算出し、`pan/tilt/x/y` を返す。実装上 `pan` と `x` は常に 0 とする。 |

引数

| No. | 引数名 | 型 | 必須 | 説明 |
|-----|--------|----|------|------|
| 1 | lstTgtUnits | List<UnitInfo> | Y | 選択Cabinet一覧（Y段数算出に使用） |
| 2 | dist | double | Y | 壁面とカメラの距離[mm] |
| 3 | wallH | double | Y | Wall下端高さ[mm] |
| 4 | camH | double | Y | カメラ高さ[mm] |
| 5 | pan(out) | double | Y | Pan角度[deg]（実装では常に0） |
| 6 | tilt(out) | double | Y | Tilt角度[deg] |
| 7 | x(out) | double | Y | X変位[mm]（実装では常に0） |
| 8 | y(out) | double | Y | Y変位[mm] |
...
| 手順No. | 処理内容 | 詳細 |
|---------|----------|------|
| 1 | 初期値設定 | `pan=0`, `x=0` を設定する。 |
| 2 | 高さ指定有無判定 | `wallH` または `camH` が `NaN` の場合、`tilt=0`, `y=0` として即 return する。 |
| 3 | 選択範囲段数算出 | `lstTgtUnits` の `Y` から `minV/maxV` を取得し、`cabiV = maxV - minV + 1` を算出する。 |
| 4 | 非選択下段数算出 | `unsel = allocInfo.MaxY - maxV` を算出する。 |
| 5 | 変位算出 | `wallV = cabinetSizeV * cabiV`、`y = (wallH - camH) + (wallV / 2) + (cabinetSizeV * unsel)` を算出する。 |
| 6 | 角度算出 | `tilt = ToDegree(Math.Atan(y / dist))` を算出して return する。 |

算出式

$$
\begin{aligned}
	ext{cabiV} &= \text{maxV} - \text{minV} + 1 \\
	ext{unsel} &= \text{allocInfo.MaxY} - \text{maxV} \\
	ext{wallV} &= \text{cabinetSizeV} \times \text{cabiV} \\
y &= (\text{wallH} - \text{camH}) + \frac{\text{wallV}}{2} + (\text{cabinetSizeV} \times \text{unsel}) \\
	ext{tilt} &= \mathrm{ToDegree}(\arctan(y / \text{dist}))
\end{aligned}
$$
...
| 区分 | 条件 | NG時挙動 |
|------|------|----------|
| 選択一覧 | `lstTgtUnits` が1件以上であること | 0件時は `minV/maxV` 由来で不正値計算の可能性 |
...
| 手順No. | 処理内容 | 詳細 |
|---------|----------|------|
| 1 | 角度変換 | `deg` をラジアンへ変換する。 |
| 2 | 回転計算 | 回転行列で `x'`,`y'` を算出する。 |
...
| 区分 | 条件 | NG時挙動 |
|------|------|----------|
| 入力値 | 数値演算可能な有限値であること | 不正値は下位演算結果に従う |
...
| 手順No. | 処理内容 | 詳細 |
|---------|----------|------|
| 1 | 基準ベクトル算出 | Cabinet の水平/垂直ベクトルを生成する。 |
| 2 | Cabinet基準角算出 | Cabinet 自体の Pan/Tilt を求める。 |
| 3 | 補正点走査 | 9点、Module、Radiator 各補正点の空間位置を計算する。 |
| 4 | 角度格納 | 各補正点の Pan/Tilt/r を `UfCamCpAngle` として返却する。 |
