...
| 呼出し先 | 役割 | 同期/非同期 |
|----------|------|--------------|
| `CaptureImage` | 補正点抽出用画像を撮影する | 同期 |
| `OutputTargetArea` / `outputTrimArea` | 対象 Cabinet とトリミング領域を表示する | 同期 |
| `CalcTrimmingAreaMask` / `MakeMaskImageArw` | マスク画像を生成する | 同期 |
...
| 呼出し先 | 役割 | 同期/非同期 |
|----------|------|--------------|
| `CaptureImage` | Radiator 補正用画像を順次撮影する | 同期 |
| `OutputTargetArea` / `outputTrimArea` | Radiator 各辺の対象領域を表示する | 同期 |
| `CalcTrimmingAreaMask` / `MakeMaskImageArw` | 各位置のマスク画像を生成する | 同期 |
| `storeCpRadiator` / `CaptureRefPoint` / `ProcRefPoint` | 補正点と基準点を抽出・整形する | 同期 |
| `MergeTrimMask` / `stopIntSig` | マスク統合と表示停止を行う | 同期 |
...
| 2 | Cabinet基準処理 | 参照画像から単一点を抽出し補正値を付与する。 |
