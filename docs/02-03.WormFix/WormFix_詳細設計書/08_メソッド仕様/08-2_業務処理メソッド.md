...
| 概要 | 指定 CSV から Unit 別補正情報を読込、補正対象 Unit に反映する。 |
...
| 手順No. | 処理内容 | 詳細 |
|---------|----------|------|
| 1 | 入力検証 | `path` と `lstTgtUnit` を確認し、読込不可時は例外または処理中断。 |
...
| 撮影工程 | 表示パターン | 用途 |
|------|------|------|
| 色変換行列用の基準撮影 | White / Red / Green / Blue / Flat / Black | RGB→LED 変換係数の算出基準画像を取得 |
| 交点検出用撮影 | Hatch 系パターン | モジュール境界・交点候補の抽出 |
| Worm検出本撮影 | タイル分割したドット/対象パターンと黒画像 | タイル単位の Worm 面積比判定 |

撮影時のカメラ設定

| 区分 | 設定内容 | 反映方法 |
|------|------|------|
| 撮影開始時 | カメラ名に応じて `m_ShootCondition` を選択（Settings由来） | 手順1で決定し、以降の `CaptureImage` へ適用 |
| 撮影中 | 測定レベル・しきい値（R/G/B）と整合する撮影条件を維持 | パターン切替ごとの撮影ループで同一方針を適用 |
| 後処理 | 撮影前のカメラ位置・ユーザー設定へ復帰 | 手順12で復帰処理を実施 |
