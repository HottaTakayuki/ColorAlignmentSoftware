...
| CAS WormFix | WORM-F02 | Worm補正（カメラ） | カメラ画像撮影・Worm検出・補正値計算 | オペレータ | 高 | detectWormAsync |
...
| 機能ID | WORM-F02 |
| 機能名 | Worm補正（カメラ）機能 |
| 機能概要 | カメラ画像撮影・Worm検出・補正値計算・進捗表示・例外復帰・ログ記録を行う |
| 利用者 | オペレータ |
...
| 計測画像ファイル | Worm補正時撮影画像 | 計測フォルダ世代管理 | 補正処理 | RAW等 |
...
|----------|--------|----------|------------------|------|
| WormFix CSV | 計測フォルダ | CSVファイル | 手動バックアップ機能提供 | WormFix専用形式 |
| 計測画像 | 計測フォルダ | 画像ファイル | 世代管理 | WormFix_yyyyMMddHHmm |
| 実行ログ | 計測フォルダ | テキスト | 世代管理 | SaveExecLog |
| 運用設定 | CAS設定 | 設定ファイル | 設定管理で保全 | Settings.Ins.WormFix |
| hc.bin | 計測フォルダ | バイナリ | 書込み時に自動生成 | Controllerへ転送 |
