...
| 8 | 結果読込処理起動 | XMLパス | 計測結果表示 | UfResultLoadService | UfCamMeasLog.LoadFromXmlFile |
...
| ファイル | UfMeasResult.xml | 読込/書込 | 計測/読込時 | path | 計測結果保存/再表示 |
...
| 手順No. | 処理内容 | 入力 | 出力 | 操作対象 | 備考 |
|---------|----------|------|------|----------|------|
| 1 | 計測レベル設定 | モデル設定 | m_MeasureLevel | 内部状態 | brightness.UF_20pc |
...
| ログ種別 | 出力条件 | 出力項目 | 保持期間 | マスキング方針 |
|----------|----------|----------|----------|----------------|
| 実行ログ | ON/OFF、主要設定適用時 | 距離条件、対象Cabinet、処理状態 | 計測フォルダ世代管理 | 機密値除外 |
...
| 手順No. | 処理内容 | 入力 | 出力 | 操作対象 | 備考 |
|---------|----------|------|------|----------|------|
| 1 | 計測フォルダ作成 | 実行日時 | measPath | ファイルシステム | UF_yyyyMMddHHmm |
...
| 対象種別 | 対象名 | 操作内容 | 操作タイミング | 主キー/識別子 | 備考 |
|----------|--------|----------|----------------|---------------|------|
| ファイル | UfMeasResult.xml | 出力 | 計測完了時 | path | 計測結果保存 |
...
| ログ種別 | 出力条件 | 出力項目 | 保持期間 | マスキング方針 |
|----------|----------|----------|----------|----------------|
| 計測ログ | 主要ステップ進行時 | ステップ名、対象、時刻、保存先 | 計測フォルダ世代管理 | 個人情報なし |
