...
| カメラ設定条件 | 実際の設定値は <br>・F値（FNumber）：F11<br>・シャッタースピード（Shutter）：1/8秒<br>・ISO感度（ISO）：100<br>・ホワイトバランス（WB）：6500K<br>・画像サイズ（ImageSize）：3（Sサイズ）<br>・圧縮形式（CompressionType）：2（JPG）<br>※（設定はSetCameraSettings/ShootCondition経由でCameraControlへ反映。F値・シャッタースピードは段階的に変更し安定化を図る。詳細はCameraControl設計書4-1-3参照） |
