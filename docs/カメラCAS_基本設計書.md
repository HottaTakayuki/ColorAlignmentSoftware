...
| CAS | CAS-08 | 目地補正機能 | Module 間の目地輝度比を解析し、目地補正値を算出する。 | オペレータ | 高 | Functions/GapCamera.cs に対応。 |
...
flowchart TD
    ...
    UI --> CameraPos[カメラ位置合わせ] & UfAdjust[U/F 調整機能] & GapAdjust[目地補正機能]
    ...
    ImageProcessGap --> GapAdjData[(目地補正データ)]
    ImageProcessGap --> GapMeasData[(目地計測データ)]
...
| CM-02 | Gap Correction(Camera) | 目地補正向けの撮影・測定操作 | オペレータ | Before/Result/Measurement を含む |
...
| CM-02-01 | 対象 Cabinet | 入力 | 構造体 | 必須 | 目地補正対象Cabinet |
...
| 目地補正 | 開始 | 表示パターンを表示した Cabinet を撮影し、画像を保存する | 画像保存、画面更新 | 撮影失敗または Controller 制御失敗を通知 |
...
| IF-CM-03 | PC ストレージ | 双方向 | ファイル I/O | 画像ファイル、CamCont.bin、ログファイルの読み書き | カメラ位置合わせ、U/F 調整、目地補正の撮影ごと、調整処理ごと |
...
| 実行契機 | カメラ位置合わせ開始、U/F 調整開始、目地補正開始 |
| 実行タイミング | オペレータ操作時 |
...
| AJ-02 | Gap Correction(Camera) | 目地補正向けの撮影・測定操作 | オペレータ | Before/Result/Measurement を含む |
...
| AJ-02-01 | 目地補正結果 | 表示 | 数値群 | - | Module 単位 |
...
| IF ID | 連携先システム | 方向 | 連携方式 | 概要 | 頻度 |
|-------|----------------|------|----------|------|------|
| IF-AJ-01 | PC ストレージ | 受信 | ファイル I/O | 画像ファイルの読み出し | U/F 調整、目地補正の処理ごと |
| IF-AJ-02 | Controller | 送信 | 装置通信 | U/F 調整値、目地補正値、関連設定値の転送 | 実行時 |
...
| 処理 | 調整・補正値生成 |
| 処理種別 | オンライン |
| 処理概要 | 撮影画像の処理結果を基に U/F 調整値および目地補正値を生成し、Controller へ転送する。 |
| 実行契機 | 撮影処理終了 |
| 実行タイミング | オペレータ操作時 |

1. 撮影画像から U/F 調整値または目地補正値を算出する。
...
| CORRECTION_VALUE | 補正データ | U/F 調整値、目地補正値を保持。 | folder_name + file_name | 実体はバイナリファイル |
...
| CAS-08 | 目地補正機能 | CORRECTION_VALUE | ○ | ○ | ○ | ○ |
...
| 撮影画像データ（RAW/JPG/MATBIN） | ログ保存先フォルダ | バイナリファイル | 設定内容に応じて、数世代前のものは自動削除される。 | U/F 調整、目地補正 |
