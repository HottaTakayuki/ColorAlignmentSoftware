...
### このファイルの差分要点
- 要件差分: Verona/Capri向けに、moire判定・Controller別gamma取得・撮影距離連動の露出補正が追加。
- I/F差分: XML+SDCP中心から、ADCP(JSON)による一括取得/一括設定が追加。
...
| 業務名 | 業務内容 | ルール・制約 |
|--------|----------|-------------|
| 位置合わせ | カメラ位置合わせを実施し計測可能状態を作る | 対象Cabinetは選択済みかつ矩形であること |
...
| 補正値バックアップ | `btnGapCamBackup_Click` / `backupGapRegAsync` | XML保存、またはADCP JSON一括取得（Verona/Capri） |
| 補正値リストア | `btnGapCamRestore_Click` / `restoreGapRegAsync` | 通常/一括書込みの2系統、またはADCP JSON一括設定（Verona/Capri） |
| gamma値取得（Verona/Capri） | `measureGapAsync` 内で Controller別に実行 | Controller毎に gamma Dictionary を構築 |
| moire判定 | `calcGapGain` 内で DFT解析により実行 | 撮影画像のmoire周波数帯を検出・判定 |
| 距離連動露出補正（Verona/Capri） | `SetCamPosTarget(..., zDistanceSpec)` で exposure を調整 | zDistanceSpec指定に応じてF値/シャッタ速度を動的設定 |
...
| CAS設定データ（Settings） | 計測レベル、撮影条件、待ち時間など運用値に影響 |
...
| 2-4-07 | 補正値リストア | XMLから補正値を読込適用 | 高 |
| 2-4-08 | 一括書込みリストア | Cell一括コマンドで高速復元 | 中 |
| 2-4-09 | 進捗表示 | 残時間・処理ステップ・中断操作を提示 | 高 |
| 2-4-10 | エラー通知 | 失敗内容を画面通知し処理を安全終了 | 高 |
| 2-4-11 | gamma値取得（Verona/Capri）| Controller別gamma値を取得し補正計算に反映 | 中 |
| 2-4-12 | moire判定 | 撮影画像をDFT解析してmoire発生判定 | 中 |
| 2-4-13 | 距離連動露出補正（Verona/Capri）| 撮影距離（zDistance）に応じてF値/シャッタ速度を動的調整 | 中 |
...
| 撮影画像 | MeasArea<br>GapPos, Top, Right<br>Gap_Before, Gap_Result<br> | 対象エリア<br>Gap輝度比計測エリア<br>Gap輝度比スイング | 計測フォルダに保存 |
...
| 連携先システム | インタフェース種別 | データ内容 | 頻度 | 備考 |
|--------------|-----------------|-----------|------|------|
| CameraControl / CameraControllerSharp | DLL呼び出し | 撮影条件、撮影画像、AF等 | 計測・位置合わせ時 | 全モデル対応 |
| Controller（SDCP） | SDCPコマンド通信 | 補正値設定、書込み、内蔵パターン制御、Reconfig | 計測・補正・書込み時 | 全モデル対応（Chiron/Cancun） |
| Controller（ADCP JSON） | JSON形式一括通信（Verona/Capri） | gamma値一括取得、補正値一括設定、補正値一括取得 | 計測・補正・バックアップ時 | Verona/Capri向け追加I/F |
| ファイルシステム | XML/画像ファイルI/O | バックアップ、リストア、計測ログ | 実行毎 | 全モデル対応 |
| CAS UI | 画面イベント/表示更新 | 開始・中断・進捗・結果表示 | 常時 | 全モデル対応 |
