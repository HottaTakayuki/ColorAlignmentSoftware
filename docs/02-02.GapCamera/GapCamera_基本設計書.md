...
### このファイルの差分要点
- 構成差分: SDCP経路に加えてADCP(JSON)経路が必要。
- 実行差分: Verona/Capriではモデル別geometry分岐とdistance依存露出補正を考慮。
...
| Settings.Ins.GapCam | 設定ストア | 撮影条件、計測レベル、待機時間など管理 | 機種別設定あり |
...
| CAS GapCamera | GAP-F06 | リストア | XML補正値を読込反映（通常/一括） | オペレータ | 高 | `restoreGapRegAsync`, `restoreBulkGapRegAsync` |
...
| 計測画像ファイル | 計測時撮影画像 | 計測フォルダ世代管理 | 計測処理 | RAW等 |
...
| 計測画像 | 計測フォルダ | 画像ファイル | 世代管理 | Gap_yyyyMMddHHmm |
| 実行ログ | 計測フォルダ | テキスト | 世代管理 | saveLog |
