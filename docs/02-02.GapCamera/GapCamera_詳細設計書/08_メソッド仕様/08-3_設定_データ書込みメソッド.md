...
### このファイルの差分要点
- 書込み差分: SDCP通常経路に加えてADCP(JSON)一括経路を追加。
- 値差分: 初期化値と2点補正/平均化ロジックの差を明記。
...
#### 8-3-9-a. Verona/Capri対応の一括書込み・読込みメソッド

**Verona/Capri向けのADCP JSON形式一括I/Fメソッド。Chiron/Cancunでは利用されない。**

| メソッド | シグネチャ | 概要 |
|---------|-----------|------|
| `setGapCellCorrectValueBulkAdcp` | `unsafe void setGapCellCorrectValueBulkAdcp(List<GapCellCorrectValue> lstGapCv)` | 全補正値をADCP JSON形式で一括設定 |
| `getGapCellCorrectValueBulkAdcp` | `unsafe void getGapCellCorrectValueBulkAdcp(out List<GapCellCorrectValue> lstGapCv)` | 全補正値をADCP JSON形式で一括取得 |
| `sendGammaValueAllGetAdcp` | `unsafe void sendGammaValueAllGetAdcp(out Dictionary<int, float> dicGamma)` | Controller別gamma値をADCP JSON形式で一括取得 |

##### 処理仕様比較

| 項目 | Chiron/Cancun（SDCP） | Verona/Capri（ADCP JSON） |
|------|------|------|
| 単位補正値設定 | `setGapCvUnit`: Cabinet毎に8辺個別コマンド | 複数Cabinet分を JSON配列で一括設定 |
| Cell補正値設定 | `setGapCvCell`: Cell毎個別コマンド | 複数Cell分を JSON構造で一括設定 |
| 処理時間 | 典型: 120秒（Module多数時） | 典型: 30秒（一括JSON） |
| 通信トラフィック | 多数回TCP接続 | 1回のJSON送受信 |
| 初期化値 | 128（Chiron/Cancun基準） | 127（Verona/Capri基準） |
| 2点補正値方式 | 直接設定 | 平均化後設定（LandR選択分岐） |

##### ADCP一括I/F成功条件

| 条件 | 詳細 |
|------|------|
| `BulkCorrectValueWall == true` | ADCP一括I/F分岐有効 |
| 対象Controller | ADCP対応モデル（Verona/Capri向けController） |
| JSON形式 | CmdGapCorrectionValueAllSet, CmdGapCorrectionValueAllGet, CmdGammaValueAllGet 実装済み |
