...
### このファイルの差分要点
- 業務処理差分: measure/adjustにgamma取得・補正反復を追加（Verona/Capri）。moire判定は両モデル共通実装。
- モデル差分: Verona/Capriのgeometry分岐と条件付き自動書込みを明記。
...
#### 8-2-1-a. measureGapAsync（Verona/Capri対応）

**Verona/Capri（Nice実装）向けの追加処理が、既存のmeasureGapAsync内で以下のように組み込まれる。**

##### gamma値取得処理の組み込み位置

| 既存手順 | Verona/Capri追加処理 | 詳細 |
|---------|------------------|------|
| 手順3 進捗・ワーク領域初期化 | → **手順3-a: Controller別gamma値取得** | 対象コントローラの全台に対して ADCP(JSON) `CmdGammaValueAllGet` を実行し、gamma値 Dictionary（Controller ID をキー）を構築 |
| 手順4 計測条件決定 | （そのまま） | gamma値 Dictionary を `m_dicGammaValue` に保持し、手順5以降の `calcGapGain` で参照 |

##### 距離連動露出補正処理の組み込み位置

| 既存手順 | Verona/Capri追加処理 | 詳細 |
|---------|------------------|------|
| 手順8 開始時カメラ姿勢取得 | → 修正: `SetCamPosTarget(..., zDistanceSpec)` | 既存: `SetCamPosTarget()` 実行のみ<br>Verona/Capri: 引数に `zDistanceSpec`（Cabinetの奥行き位置を用いた判定値）を指定、内部で `ShootCondition.ForNice` 分岐により F値/シャッタ速度を動的調整 |
| 手順9 撮影処理 | （そのまま） | 動的調整されたF値/SSで撮影実行 |

##### 補正計算への gamma反映

| 項目 | 詳細 |
|------|------|
| 関数 | `calcGapGain(lstTgtUnit, m_CamMeasPath)` 内で実施 |
| 処理 | 手順10「解析処理」で、Controller別 gamma値を参照して色変換マトリックス計算に補正係数を追加 |
...
#### 8-2-2-a. adjustGapRegAsync（Verona/Capri対応）

**Verona/Capri（Nice実装）向けの追加処理が、既存のadjustGapRegAsync内で以下のように組み込まれる。**

##### gamma値取得フロー

| 処理段階 | 詳細 |
|---------|------|
| 計測時タイミング | `measureGapAsync` の手順4「計測条件決定」実行時に、対象コントローラ別に gamma値を一括取得 |
| 取得方法 | ADCP(JSON) CmdGammaValueAllGet により、各Controller の現在gamma値を Dictionary に格納 |
| 用途 | `calcGapGain` の補正計算時に、gamma値を考慮した色変換マトリックス調整に利用 |
| 例外処理 | gamma値取得失敗時は例外を送出し、計測を中断（Verona/Capri向けエラー） |

##### moire判定フロー

| 処理段階 | 詳細 |
|---------|------|
| 判定タイミング | `calcGapGain` 内で撮影画像を DFT（離散フーリエ変換）で解析 |
| 判定ロジック | `calcMoireCheckArea` と `checkMoire` メソッドで、moire周波数帯（例: 200-500 pixel周期）を検出 |
| 判定結果 | moire発生判定を `GapCamCp.m_MoireDetected` に記録 |
| 後続処理 | moire判定が陽性の場合、`adjustGapRegAsync` の補正ループ内で追加反復補正を実施 |

##### 距離連動露出補正フロー

| 処理段階 | 詳細 |
|---------|------|
| 設定タイミング | `SetCamPosTarget(..., zDistanceSpec)` 実行時に、撮影距離に応じて exposure パラメータを動的調整 |
| 補正対象 | F値（Aperture）とシャッタ速度（Shutter Speed） |
| 補正ロジック | `ShootCondition` クラス内の ForNice 条件分岐により、zDistance → F値/SS マッピングを実行 |
| 適用モデル | VP15EB/EM (4×4 Module), VP23EB/EM (4×4 Module), VS25FM (2×2 Module) にて異なる補正係数を適用 |

##### 反復補正評価ロジック

| 項目 | Chiron/Cancun | Verona/Capri |
|------|---------------|--------------|
| 規格判定基準 | `AdjustSpec` 固定値 | `Spec_by_Zdistance` にて Cabinet位置（Z距離）別に スペック変更 |
| 判定タイミング | 手順9で全点比較 | 手順9に加え、 moire判定結果を反映した 追加判定ロジック |
| 補正ループ条件 | `GapGain < AdjustSpec` で継続 | 上記に加え `m_MoireDetected == true` で補正継続 |
| 早期終了条件 | 全点規格内達成 | 全点規格内 **且つ** moire未検出 |

##### 書込方式の選択

| 項目 | Chiron/Cancun | Verona/Capri |
|------|---------------|--------------|
| 標準書込み | SDCP CmdGapCorrectionValue 単発コマンド | SDCP で個別書込み（後方互換性） |
| 一括書込み（BulkCorrectValueWall） | ×（未対応） | ○ ADCP CmdGapCorrectionValueAllSet で一括反映、その後 `Auto_WriteData` で ROM書込み自動実行 |
...
| `cmbxPatternGapCam.SelectedIndex` | 計測レベルに応じて更新 | 書込み後 |
...
#### 8-2-4-a. backupGapRegAsync（Verona/Capri対応）

**Verona/Capri（Nice実装）では、従来のXML保存に加え、ADCP JSON形式での一括取得が追加される。**

##### 取得方式の選択フロー

| 取得方式 | 適用条件 | 実装詳細 |
|---------|---------|---------|
| XML標準保存（従来） | 常に実行 | 手順1〜6は既存ロジックで継続 |
| ADCP JSON一括取得 | `BulkCorrectValueWall == true` | 手順5で `getGapCvCell` コマンド実行の代わり、`CmdGapCorrectionValueAllGet` (JSON)を実行、全Cellを一括取得 |

##### 実装変更点

| 項目 | Chiron/Cancun | Verona/Capri |
|------|---------------|--------------|
| 読出しコマンド | SDCP `CmdGapCorrectionValue` 単発×(Module数) | SDCP `CmdGapCorrectionValue` OR<br>ADCP(JSON) `CmdGapCorrectionValueAllGet`（高速） |
| 処理時間短縮 | ×（個別コマンド） | ○ 一括取得で処理時間 最大50%削減 |
| 保存形式 | XML のみ | XML + JSON（BulkCorrectValueWall有効時） |
...
#### 8-2-6-a. restoreBulkGapRegAsync（Verona/Capri対応）

**Verona/Capri（Nice実装）では、一括復元時に ADCP JSON形式の高速設定が追加される。**

##### 一括設定方式の選択フロー

| 設定方式 | 適用条件 | 実装詳細 |
|---------|---------|---------|
| SDCP単発設定（従来） | 常に実行可能 | 手順3で `setGapCvCellBulk` を実行、各Moduleを順次設定 |
| ADCP JSON一括設定（高速） | `BulkCorrectValueWall == true` かつ ADCP対応 | 手順3で代わりに `CmdGapCorrectionValueAllSet` (JSON)を実行、全Cell一括設定後、`Auto_WriteData` で ROM自動書込み |

##### 処理時間短縮効果

| 項目 | Chiron/Cancun | Verona/Capri（ADCP） |
|------|---------------|-----------------|
| 設定方式 | SDCP単発×(Module数) | ADCP JSON一括 |
| 処理時間 | 典型: 120秒 | 典型: 30秒（75%削減） |
| ROM書込み | `writeGapCellCorrectionValueWithReconfig` | `Auto_WriteData` で自動実行可 |
...
| 撮影工程 | 表示パターン | 用途 |
|------|------|------|
| 黒画像取得 | `outputIntSigFlat` | 反射成分差分用の黒基準画像を取得 |
| フラット/Gap画像取得 | `outputIntSigFlatGap` | レベル別 Gap 画像（Flat/Gap）を取得 |
| Trimming領域取得 | `outputIntSigHatch` / `outputIntSigHatchInv` / `outputGapCamTargetArea_EdgeExpand` | Top/Right/GapPos のトリミング対象を強調表示 |
| 対象エリア復帰 | `outputGapCamTargetArea*` 系 | 解析入力用の表示状態へ復帰 |

撮影時のカメラ設定

| 区分 | 設定内容 | 反映方法 |
|------|------|------|
| 通常撮影 | 機種別 `m_ShootCondition`（`Settings.Ins.GapCam.SettingA6400` / `SettingA7`） | `CaptureImage(imgPath)` または `CaptureImage(imgPath, m_ShootCondition)` |
| 位置/距離連動撮影（Verona/Capri） | `SetCamPosTarget(..., zDistanceSpec)` による F値/シャッター速度補正後の `ShootCondition` | `ShootCondition.ForNice` 分岐適用後に撮影 |
| 運用上の共通手順 | パターン表示後に待機して撮影を実行 | 「表示出力→`Thread.Sleep(PatternWait)`→`CaptureImage(...)`」 |
