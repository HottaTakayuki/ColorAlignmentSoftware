...
### このファイルの差分要点
- 演算差分: gamma、Spec_by_Zdistance、CorrectTargetEdgeの影響を明記。
### 更新時の注意
...
本章の OpenCV/画像処理メソッドは、以下の実装を正本として記載している。

| メソッド | ソースコード参照 |
|------|------|
| `captureGapImages` | `CAS/Functions/GapCamera.cs:6133` |
| `calcGapGain(List<UnitInfo>, string)` | `CAS/Functions/GapCamera.cs:7739` |
| `makeTargetArea` | `CAS/Functions/GapCamera.cs:8175` |
| `getTrimmingAreaGap` | `CAS/Functions/GapCamera.cs:8521` |
| `calcCpGainRaw` | `CAS/Functions/GapCamera.cs:10287` |
| `calcGapPos` | `CAS/Functions/GapCamera.cs:10518` |
| `adjustGapRegAsync`（関連） | `CAS/Functions/GapCamera.cs:12334` |
| `setGapCorrectValue`（関連） | `CAS/Functions/GapCamera.cs:14042` |

#### 8-4-0. 撮影関連メソッド補足（表示パターン/カメラ設定）

本章の解析メソッド（特に `calcGapGain` / `calcCpGainRaw`）は、撮影済み画像を入力として扱う。撮影そのものは `captureGapImages`（8-2章）で実行されるため、撮影時の表示パターンとカメラ設定を本節に補足する。

撮影時の表示パターン

| 撮影工程 | 主な表示パターン | 用途 |
|------|------|------|
| 反射判定用黒画像 | `outputIntSigFlat` | 黒基準画像の取得 |
| Gap計測画像 | `outputIntSigFlatGap` | レベル別 Gap 画像の撮影 |
| トリミング領域取得 | `outputIntSigHatch` / `outputIntSigHatchInv` / `outputGapCamTargetArea_EdgeExpand` | Top/Right/GapPos の領域抽出 |

撮影時のカメラ設定

| 区分 | 設定内容 | 反映方法 |
|------|------|------|
| 通常撮影 | 機種別 `m_ShootCondition`（`Settings.Ins.GapCam` 系設定） | `CaptureImage(imgPath)` または `CaptureImage(imgPath, m_ShootCondition)` |
| 距離連動補正（Verona/Capri） | `SetCamPosTarget(..., zDistanceSpec)` による F値/シャッター速度の動的補正 | `ShootCondition.ForNice` 分岐で条件更新後に撮影 |

※ 呼出し順は「表示パターン出力 → `Thread.Sleep(PatternWait)` → `CaptureImage(...)`」を基本とする。
...
#### 8-4-5-a. Verona/Capri対応の演算差分

本メソッド以降、以下のメソッドは Verona/Capri固有の演算または既存機能の拡張を含む。

| 対象メソッド | Verona/Capri固有の内容 | 備考 |
|-------------|----------------------|------|
| `calcGapGain` 関連 | gamma値反映、Spec_by_Zdistance | 既存メソッドへの拡張 |
| `calcNewRegUnit`, `calcNewRegCell` | CorrectTargetEdge考慮 | 補正対象の選択肢拡張 |
| 他メソッド | 初期値差(127 vs 128) | Chiron/Cancun: 128, Verona/Capri: 127初期 |

---

#### 8-4-6. calcMoireCheckArea

**本メソッドはmoire判定機構として、両モデル（Chiron/Cancun、Verona/Capri）で実装される。**
...
| 2 | 半タイル画像読込（任意） | `MultiController` かつ半タイル有効時は `file + "_Half"` を読込、後段の差分加算に使用する。 |
...
#### 8-4-x. estimateCamPos

| 項目 | 内容 |
|------|------|
| シグネチャ | `private void estimateCamPos(CvBlob[,] aryBlob)` |
| 概要 | タイル検出結果からカメラのPan/Tilt/RollおよびX/Y/Z位置を推定し、内部状態へ反映する。 |
| 引数 | aryBlob: タイル検出済みの2次元CvBlob配列 |
| 返り値 | なし（void） |

**処理詳細**
1. タイル配列 `aryBlob` から各コーナー（左上・右上・右下・左下）の画像座標を抽出。
2. Cabinet配置情報・カメラパラメータ・既知の幾何情報を用いて、
   - Pan（水平回転）、Tilt（上下回転）、Roll（傾き）
   - X/Y/Z（カメラ中心座標）
   を幾何計算で推定。
3. 推定値を `m_CamPos_Pan`、`m_CamPos_Tilt`、`m_CamPos_Roll`、`m_CamPos_Tx`、`m_CamPos_Ty`、`m_CamPos_Tz` などの内部変数へ格納。
4. Cabinet座標系への変換や、必要に応じて `MoveCabinetPos` などの後続処理を呼び出す。

**主要呼出し先・依存**
- Cabinet配置情報（allocInfo.lstUnits等）
- カメラパラメータ（m_CameraParam等）
- 幾何変換ユーティリティ（TransformImage等）
- MoveCabinetPos（座標変換）

**入力条件・前提**
- `aryBlob`が正しくタイル検出済みであること
- Cabinet配置・カメラパラメータが初期化済みであること

**例外時仕様**
- 入力配列不正や計算失敗時は例外送出、呼出元でエラー通知・UI復帰

**備考**
- Pan/Tilt/Roll・X/Y/Z推定は、画像上のタイル配置と実際のCabinet配置の対応から逆算する。
- Cabinet配置やカメラパラメータが変化した場合は再推定が必要。

**シーケンス図（呼出し例）**
```mermaid
sequenceDiagram
    autonumber
    participant M as measureGapAsync/AdjustCameraPosition
    participant ANA as estimateCamPos
    M->>ANA: estimateCamPos(aryBlob)
    ANA->>ANA: 幾何計算・姿勢推定
    ANA-->>M: Pan/Tilt/Roll・X/Y/Z反映
```
