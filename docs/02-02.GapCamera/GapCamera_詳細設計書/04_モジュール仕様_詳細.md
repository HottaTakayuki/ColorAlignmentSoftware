...
### このファイルの差分要点
- 詳細手順差分: measure/adjust/backup-restoreでVerona/Capriの分岐が増加。
- 例外差分: camera position不適合、gamma取得失敗、moire検出を明記。
...
| ファイル | 計測ログ | 出力 | 実行中 | 日時フォルダ | `saveLog` |
...
|----------|----------|----------|----------|----------------|
| 実行ログ | 処理開始/終了/主要ステップ | 時刻、処理名、進捗 | 計測フォルダ世代管理 | 個人情報なし |
...
| 手順No. | 処理内容 | 入力 | 出力 | 操作対象 | 備考 |
|---------|----------|------|------|----------|------|
| 1 | 計測レベル設定 | モデル名 | m_MeasureLevel | Settings | B/Cモデル分岐 |
...
| ログ種別 | 出力条件 | 出力項目 | 保持期間 | マスキング方針 |
|----------|----------|----------|----------|----------------|
| 実行ログ | ON/OFF、主要設定適用時 | LEDモデル、設定値、処理状態 | 計測フォルダ世代管理 | 機密値除外 |
...
| 手順No. | 処理内容 | 入力 | 出力 | 操作対象 | 備考 |
|---------|----------|------|------|----------|------|
| 1 | 計測フォルダ作成 | 実行日時 | `m_CamMeasPath` | ファイルシステム | Gap_yyyyMMddHHmm |
...
| 4 | moire検出 | `checkMoire` 判定 | 計測継続可能 | 警告表示 | saveLog | 可（両モデル） |
| 5 | カメラ位置不適合（Verona/Capri） | `GetCameraPosition()` 姿勢判定 | 処理中断 | CAS Error | saveLog | 不可（Developerモード除く） |
| 6 | gamma値取得失敗（Verona/Capri） | ADCP通信例外 | 処理中断 | CAS Error | saveLog | 可 |

#### 4-3-6-a. Verona/Capri対応の処理詳細

| 項目 | Chiron/Cancun | Verona/Capri |
|------|---------------|--------------|
| 計測前処理 | 撮影条件設定 + AF | 撮影条件設定 + gamma値取得（Controller別） + 距離連動露出補正 + AF |
| gamma値取得 | × | ○ 手順4「画像取得」前に ADCP `CmdGammaValueAllGet` 実行 |
| 距離連動露出補正 | × | ○ `SetCamPosTarget(..., zDistanceSpec)` で F値/SS動的調整 |
| 解析内容 | Gap輝度比算出 + moire判定（DFT解析） | Gap輝度比算出 + moire判定（DFT解析） |
| moire判定 | ○ `calcMoireCheckArea` → `checkMoire` により周波数解析 | ○ `calcMoireCheckArea` → `checkMoire` により周波数解析 |

#### 4-3-7. ログ仕様

| ログ種別 | 出力条件 | 出力項目 | 保持期間 | マスキング方針 |
|----------|----------|----------|----------|----------------|
| 計測ログ | 主要ステップ進行時 | ステップ名、対象、時刻 | 計測フォルダ世代管理 | 個人情報なし |
...
| ファイル | 実行ログ | 追記 | 補正中 | 計測フォルダ | stepログ |
...
| 4 | moire検出 | `m_MoireDetected == true` | 補正ループ継続 | 警告表示 | saveLog | 継続（両モデル） |
| 5 | カメラ位置不適合（Verona/Capri） | `GetCameraPosition()` 姿勢判定 | 処理中断 | CAS Error | saveLog | 不可（Developerモード除く） |

#### 4-4-6-a. Verona/Capri対応の反復補正ロジック

| 項目 | Chiron/Cancun | Verona/Capri |
|------|---------------|--------------|
| 補正継続条件 | `GapGain >= AdjustSpec` または `m_MoireDetected == true` | `GapGain >= Spec_by_Zdistance` または `m_MoireDetected == true` |
| 規格判定 | AdjustSpec（固定値）+ moire判定 | `Spec_by_Zdistance`（Cabinet位置別） + moire判定 |
| 反復補正ループ | 最大 `m_MaxNumOfAdjustment` 回（moire除去条件含む） | 最大 `m_MaxNumOfAdjustment` 回（moire除去条件含む） |
| 補正値書込み方式 | SDCP単発 | SDCP単発 または ADCP JSON一括（BulkCorrectValueWall） |
| 自動書込み | × | ○ `Auto_WriteData == true` 時に補正ループ後自動実行 |

#### 4-4-7. ログ仕様

| ログ種別 | 出力条件 | 出力項目 | 保持期間 | マスキング方針 |
|----------|----------|----------|----------|----------------|
| 補正ログ | 補正ステップ進行時 | ステップ、対象、補正値 | 計測フォルダ世代管理 | 機密値除外 |
...
| 4 | ADCP通信失敗（Verona/Capri） | JSON形式送受信例外 | 処理停止 | CAS Error | 任意 | 可 |

#### 4-5-6-a. Verona/Capri対応のバックアップ/リストア処理

| 項目 | Chiron/Cancun | Verona/Capri |
|------|---------------|--------------|
| バックアップ方式 | XML一括保存（SDCP単発読出） | XML保存 ＋ JSON形式オプション（ADCP一括読出） |
| リストア方式（通常） | XML読込→SDCP単発設定→Write | 同上 |
| リストア方式（一括） | XML読込→SDCP Bulk設定→Write | XML読込→ADCP JSON一括設定→`Auto_WriteData` |
| 処理時間 | 120秒程度（SDCP単発） | 30秒程度（ADCP JSON一括） |
| 初期値（初期化値） | 128（固定） | Chiron/Cancun: 128 vs Verona/Capri: 127（モデル別） |

#### 4-5-7. ログ仕様

| ログ種別 | 出力条件 | 出力項目 | 保持期間 | マスキング方針 |
|----------|----------|----------|----------|----------------|
| 実行ログ | Backup/Restore開始・終了 | path、件数、結果 | 計測フォルダ世代管理 | 個人情報なし |
...
| 4 | ADCP通信失敗（Verona/Capri） | JSON形式送受信例外 | 処理停止 | CAS Error | 実行ログ | 可 |
| 5 | Auto_WriteData失敗（Verona/Capri） | 書込み応答例外 | 処理停止 | CAS Error | 実行ログ | 可 |

#### 4-6-6-a. Verona/Capri対応の書込み処理

| 項目 | Chiron/Cancun | Verona/Capri |
|------|---------------|--------------|
| 標準書込み流れ | Panel OFF→SDCP単発Write→Reconfig→Panel ON | 同上（後方互換性） |
| 高速書込み流れ | ×（未対応） | ○ Panel OFF→ADCP JSON一括Set→`Auto_WriteData`実行→Panel ON |
| 自動書込み分岐 | × | ○ `BulkCorrectValueWall == true` 時に ADCP一括→Auto_WriteData実行 |
| 処理時間 | 標準: 40秒 | 標準: 40秒、高速: 10秒（ADCP使用時） |

#### 4-6-7. ログ仕様

| ログ種別 | 出力条件 | 出力項目 | 保持期間 | マスキング方針 |
|----------|----------|----------|----------|----------------|
| 実行ログ | Write開始〜終了 | Step、対象Cabinet、結果 | 計測フォルダ世代管理 | 機密値除外 |
