...
実行時は CameraControllerSharp.dll を参照し、CameraController.lib は CameraControllerSharp.dll のビルド時に静的リンク済みのため別途配置対象としない。
...
| 2 | CameraControllerSharp.dll | 外部SDK | カメラデバイス制御本体 | x86依存。CameraController.lib をビルド時に静的リンク済み |
...
| 画像解析 | ライブビュー解析はOpenCvSharpに委譲 |
| 配置方針 | 実行時配布物は CameraControllerSharp.dll を対象とし、CameraController.lib の個別配布は不要 |
