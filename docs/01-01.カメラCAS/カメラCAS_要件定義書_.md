...
| 項目 | 内容 |
|------|------|
| カメラCAS | カメラを使って、LDSのU/F調整、Gap補正を行う |
| 作成日 | 2026/4/14 |
...
・Cabinet/Module間の距離によって見える明線/暗線を信号処理で見えなくするGap補正
といった作業が必要である。
LDSは、数十台以上の大量のCabinetで構成され、サイズも大型であるため、人による目視作業でこれらの作業を行うのは負担が大きく、時間もかかる。
そこで、Cabinetをカメラで撮影し、画像解析でU/F調整及びGap補正を自動処理するシステムを導入し、短時間で品質の良い作業結果を提供する。
...
| Gap補正 |Cabinet/Module間の距離のバラツキによる明線/暗線を見えないように補正する |  |
...
| Gap補正 | 内蔵パターンを表示したCabinetをカメラで撮影し、Module間のGap輝度比がゼロになるGap補正値を自動算出し、Controllerに転送する | Gap輝度比：隣り合うModule端の明るさ比。ゼロになると、Gap（明線及び暗線）が見えない |
...
| その他 | 暗環境でなくても使用できること<br>U/F調整は1回あたり最大8K4Kサイズを調整できること<br>Gap補正は1回あたり最大4K2Kサイズを補正できること |
...
本システムの適用範囲は、LDSの画質調整作業のうち、U/F調整とGap補正とする。
...
| 画像解析 | 撮影画像からのCabinet/Module単位の明るさの取得、Module間Gap輝度比の取得 |
| 補正値算出 | U/F調整値およびGap補正値の自動算出 |
...
| 画像解析方針 | LDSを撮影した画像からCabinet/Module単位の明るさ、Gap輝度比を算出し、U/F調整およびGap補正に必要な補正値を自動生成する。 |
...
| 品質確保方針 | カメラ位置、撮影条件、表示パターンを標準化し、同一条件で再現性のある計測結果を取得できるようにする。 |
...
| 2-4-05 | Module明るさ 解析機能 | 撮影画像からModule単位の明るさを算出できること。 | 高 |
| 2-4-06 | Module間Gap輝度比 解析機能 | 撮影画像からModule間のGap輝度比を算出できること。 | 高 |
| 2-4-07 | U/F調整値算出機能 | 明るさの解析結果に基づき、目標範囲に収まるU/F調整値を自動算出できること。 | 高 |
| 2-4-08 | Gap補正値算出機能 | Gap輝度比の解析結果に基づき、Gap輝度差を低減するGap補正値を自動算出できること。 | 高 |
...
| Gap輝度比解析結果データ | Cabinet/Module間識別子、Gap輝度比 | 撮影画像データ、Gap補正値データ | Gap補正値算出の元データとする。 |
| U/F調整値データ | Cabinet/Module識別子、調整前値、調整後値、 | 明るさ解析結果データ、実行履歴データ | Controllerへの転送対象データとする。 |
| Gap補正値データ | Cabinet/Module間識別子、補正値 | Gap輝度比解析結果データ、実行履歴データ | Controllerへの転送対象データとする。 |
| 実行履歴データ | 実行日時、対象情報、実行機能、実行結果 | 測定条件データ、解析結果データ、ログデータ | トレーサビリティ確保、障害調査、運用保守のため保存する。 |

データの関係性は以下を基本とする。

1. 撮影条件データを基に撮影し、撮影画像データを生成する。
2. 撮影画像データを基に明るさ解析結果データ、Gap輝度比解析結果データを生成する。
3. 解析結果データを基にU/F調整値データ、Gap補正値データを生成する。
...
|--------------|-----------------|-----------|------|
| カメラ | USB接続 | 撮影開始指示、撮影条件、撮影画像データ | 測定実行時 |
| Controller | Ethernet | 内蔵パターン表示指示、U/F調整値、Gap補正値 | 測定実行時 |
...
| 正確性 | 画像解析結果および補正値算出結果が品質要求に適合すること。 | U/F調整後のModule間輝度差が±1.7%以内、Gap補正後のModule間Gap輝度比が±1.5%以内 |
...
| データ処理時間 | 撮影画像の取り込みから解析、補正値算出までを実用時間内で完了できること。 | UF調整の処理時間が30分以内、Gap補正の処理時間が30分以内 |
